...
export async function prepareSignupProfile(data: SignupProfileInput) {
  const input = signupProfileSchema.parse(data);

  if (input.role === UserRole.DEVELOPER && !input.companyName?.trim()) {
    throw new Error("Company or studio name is required for developer accounts.");
  }
